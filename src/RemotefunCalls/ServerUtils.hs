...
import RemotefunCalls.DeclsGenerator (genServer)
...
serveRPC host portNum rpcTable = 
    serve (Host host) (show portNum) procRequests
        where
...
serveRequest rpcTable = 
    receiveRSIO >>= call >>= sendRSIO
        where
